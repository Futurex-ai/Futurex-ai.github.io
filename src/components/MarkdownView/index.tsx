// src/components/MarkdownView/index.tsx
...
// 从原始 markdown 抽取 TOC（只抓 h2~h4；你已移除 H1，避免与 Banner 冲突）
...
function parseTOC(md: string): TocItem[] {
  ...
  for (const ln of lines) {
    ...
    const text = cleanHeadingText(rawText); // ★ 清洗掉 ** 等内联标记
    if (!text) continue;
    const id = slugify(text); // ★ id 用清洗后的文本，确保与组件渲染一致
...
export const MarkdownView: React.FC = () => {
  ...
  const tableStyle: React.CSSProperties = {
    ...
    border: "none", // ★ 清掉 github-markdown-css 自带 table 外边框，避免与 wrapper 叠加
  };
  const thStyle: React.CSSProperties = {
    position: "sticky" as const,
    top: 0,
    background: "#f9fafb",
    border: "none", // ★ 先清零再加需要的边
  ...
  const components = {
    ...
    p: ({ node, children, ...props }: any) => {
      ...
      if (isOnlyImg) {
        ...
        const needs90 = rawSrc.includes("20250805150617761");
        return (
          <figure className="fx-figure" {...props}>
            <img
              ...
              src={resolvedSrc}
              alt={alt}
              style={needs90 ? { width: "90%" } : undefined}
    ...
    img: ({ src = "", alt = "", ...props }: any) => {
      ...
      const needs90 = String(src).includes("20250805150617761");
      return (
  ...
  return (
    <div style={containerStyle} className={`fx-article-shell ${toc.length ? "has-toc" : ""}`}>
      ...
      <article className="markdown-body fx-article">
        <ReactMarkdown
          // 你已经把 markdown 里的 # H1 去掉了，避免和 Banner 重合
